...
class MCAT_Surv(nn.Module):
    def __init__(self, fusion='concat', omic_sizes=[100, 200, 300, 400, 500, 600], n_classes=4,
        ...
        sig_networks = []
        for input_dim in omic_sizes:    # x_omic的大小为4
            fc_omic = [SNN_Block(dim1=input_dim, dim2=hidden[0])]
            for i, _ in enumerate(hidden[1:]):  # 事实上只有256
    ...
    def forward(self, **kwargs):
        ...
        hazards = torch.sigmoid(logits) # p(T=t|T>=t,X)
        S = torch.cumprod(1 - hazards, dim=1)   # p(t>=T,X)
        
        attention_scores = {'coattn': A_coattn, 'path': A_path, 'omic': A_omic}
        return hazards, S, Y_hat, attention_scores, None, None
...
# pytorch 1.10版本后没有_LinearWithBias
